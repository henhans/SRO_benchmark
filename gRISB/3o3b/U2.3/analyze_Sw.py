...
def compute_Gf_Sig(mu, ek_path, oms, eta, R, Lambda, eloc, nbath, nimp, ntot):
    ...
    for iom, om in enumerate(oms):
        ...
        for ik, ek in enumerate(ek_path):
            ...
            if ik == 0:
                Sig[iom,:,:] = ( (om + 1j*eta + mu)*np.eye(nimp) - ek - eloc) - inv(Gf[iom,:,:])
...
for U in [2.3]:#Us:
    ...
    plt.plot(oms, Aw[:,4,4] + Aw[:,5,5] , 'g-', label='Ru-dyz')
    plt.plot(oms, Aw[:,2,2] + Aw[:,3,3] , 'r-', label='Ru-dxz')
    plt.plot(oms, Aw[:,0,0] + Aw[:,1,1] , 'b-', label='Ru-dxy')
    plt.axvline(0,color='k',ls='-')
    plt.ylim(0,)
    plt.legend(loc='best')
    plt.show()

    np.savetxt('Sw_xy.dat',np.vstack((oms,Sig[:,0,0].real,Sig[:,0,0].imag)).T)
    np.savetxt('Sw_xz.dat',np.vstack((oms,Sig[:,2,2].real,Sig[:,2,2].imag)).T)
    np.savetxt('Sw_yz.dat',np.vstack((oms,Sig[:,4,4].real,Sig[:,4,4].imag)).T)
